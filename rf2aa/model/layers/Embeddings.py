...
class Templ_emb(nn.Module):
    ...
    def forward(self, t1d, t2d, alpha_t, xyz_t, mask_t, pair, state, use_checkpoint=False, p2p_crop=-1, is_prot=None):
        ...
        if ipd.symmetrize.copy_main_block_template:
            assert ipd.symmetrize.main_block is not None
            assert ipd.symmetrize.symmetrize_repeats
            ...
            templ_copy_out = copy_main_2d(templ_to_copy, self.repeat_length, ipd.symmetrize.main_block)
        ...
        if ipd.symmetrize.copy_main_block_template:
            ...
            t1d_copied = copy_main_1d(t1d_to_copy, self.repeat_length, ipd.symmetrize.main_block)
            # copy back into the full representation
...
class Templ_emb_NoPtwise(nn.Module):
    ...
    def forward(self, t1d, t2d, alpha_t, xyz_t, mask_t, pair, state, use_checkpoint=False, p2p_crop=-1):
        ...
        if ipd.symmetrize.copy_main_block_template:
            ...
            assert ipd.symmetrize.symmetrize_repeats
            # copy the main repeat unit internally down the pair representation diagonal 
            templ = copy_main_2d(templ, self.repeat_length, ipd.symmetrize.main_block)
        ...
        if ipd.symmetrize.copy_main_block_template:
            # already made assertions above 
            # copy main unit down single rep 
            t1d = copy_main_1d(t1d, self.repeat_length, ipd.symmetrize.main_block)
