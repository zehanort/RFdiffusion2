...
class PairStr2Pair(nn.Module):
    ...
    def forward(self, pair, rbf_feat, state, crop=-1, is_prot=None):
        ...
        for i in range(len(pair)):
            pair[i] = ipd.symmetrize(pair[i:i + 1])
...
class IterBlock(nn.Module):
    ...
    def forward(self,
        ...
        xyz, pair = ipd.symmetrize(xyz, pair, update_symmsub=True)
        xyz = ipd.motif_applier(xyz)
...
class IterativeSimulator(nn.Module):
    ...
    def forward(self,
        ...
        for i_m in range(self.n_extra_block):
            ...
            ipd.hub.rfold_iter_begin(f'extra_block_{i_m:02}')
            ...
            quat_s.append(quat)
            ipd.hub.rfold_iter_end(name=f'extra_block_{i_m:02}', xyz=xyz)
        ...
        for i_m in range(self.n_main_block):
            ...
            if self.use_chiral_l1:
                dchiraldxyz, = calc_chiral_grads(xyz.detach(), chirals)
                extra_l1 = dchiraldxyz[0].detach()
            ipd.hub.rfold_iter_begin(f'main_block_{i_m:02}')
            ...
            alpha_s.append(alpha)
            quat_s.append(quat)
            ipd.hub.rfold_iter_end(name=f'main_block_{i_m:02}', xyz=xyz)
        ...
        for i_m in range(self.n_ref_block):
            with ExitStack() as stack:
                ...
                extra_l0 = None
                extra_l1 = []
                ipd.hub.rfold_iter_begin(f'ref_block_{i_m:02}')
                ...
                xyz = ipd.symmetrize(xyz)
                xyz = ipd.motif_applier(xyz)

                xyz_s.append(xyz)
                alpha_s.append(alpha)
                quat_s.append(quat)
                ipd.hub.rfold_iter_end(name=f'ref_block_{i_m:02}', xyz=xyz)
