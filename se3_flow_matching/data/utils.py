...
from se3_flow_matching.data import residue_constants
import numpy as np
import collections
import string
import pickle
...
from se3_flow_matching.data import protein
import dataclasses
from Bio import PDB
...
def center_zero(pos: torch.Tensor, batch_indexes: torch.LongTensor) -> torch.Tensor:
    ...
    # means1 = scatter(pos, batch_indexes, dim=0, reduce="mean")
    # Hack to avoid importing scatter
    means = pos.mean(dim=0)[None]
    # assert torch.linalg.vector_norm(means-means1) < 0.01
...
def align_structures(
    ...
    # # FOR TESTING: DELETE
    # reference_positions += torch.rand(*batch_positions.shape)
    ...
    # )
    src = batch_positions[:, None, :] * reference_positions[:, :, None]
    cov = torch.zeros((torch.max(batch_indices)+1, 3, 3), dtype=src.dtype, device=src.device)
    cov.scatter_reduce_(dim=0, index=batch_indices[:,None,None].repeat(1,3,3), src=src, reduce='sum', include_self=False)
    # assert torch.linalg.vector_norm(cov-cov1) < 0.01
