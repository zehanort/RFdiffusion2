...
os.environ["HF_HOME"] = str(Path(__file__).resolve().parent.parent.parent / 'chai_cache')
...
def make_serializable(obj):
    ...
    if isinstance(obj, torch.Tensor):
        ...
        # Recursively process lists or tuples
        return [make_serializable(item) for item in obj]
    elif isinstance(obj, dict):
        # Recursively process dictionaries
        return {key: make_serializable(value) for key, value in obj.items()}
    elif hasattr(obj, '__dict__'):
        # Convert objects with __dict__ (custom objects) to a serializable format
...
def run_inference(
    ...
    output_cif_paths, confidence_scores, ranking_data, msa_plot_path = run_folding_on_context(
        feature_context,
    ...
    if export_jsons:
        ...
        for ranking in ranking_data:
            rank_dict = {}
            for i in ranking.__dict__.keys():
                rank_dict[i] = make_serializable(ranking.__dict__[i])
            ranking_list.append(rank_dict) 
            
        scores_dicts = {}
        for i in confidence_scores.__dict__.keys():
            scores_dicts[i] = confidence_scores.__dict__[i].tolist()
        
        import json
        json.dump(make_serializable(output_cif_paths), open(f'{output_dir}/{name}_output_paths.json', 'w'))
        json.dump(ranking_list, open(f'{output_dir}/{name}_ranking_data.json', 'w'))
        json.dump(scores_dicts, open(f'{output_dir}/{name}_scores.json', 'w'))
...
def run_folding_on_context(
    ...
    if args.save_latents:
        single_out_path = output_dir.joinpath(f"{name}_single.pt")
        pair_out_path = output_dir.joinpath(f"{name}_pair.pt")
        torch.save(token_single_trunk_repr.detach().cpu(), single_out_path)
        torch.save(token_pair_trunk_repr.detach().cpu(), pair_out_path)
    ...
    for idx in range(num_diffn_samples):
        ...
        if 'cif' in args.structure_output:
            cif_out_path = output_dir.joinpath(f"pred.{name}_model_idx_{idx}.cif")
        ...
        if 'pdb' in args.structure_output:
            pdb_out_path = output_dir.joinpath(f"pred.{name}_model_idx_{idx}.pdb")
        ...
        scores_basename = f"scores.{name}_model_idx_{idx}.npz"
        scores_out_path = output_dir / scores_basename

        scores = get_scores(ranking_outputs)
        
        if args.export_arrays:
            scores['pae_array'] = confidence_scores.pae[idx].cpu().numpy()
            scores['plddt_array'] = confidence_scores.plddt[idx].cpu().numpy()
            
        if args.export_seed:
            scores['seed'] = seed
        
        if 'json' in args.export_mode:
            with open(f"{str(scores_out_path)[:-4]}.json", "w") as f:
                import json
                def convert_ndarray_to_list(obj):
                    if isinstance(obj, np.ndarray):
                        ...
                        return obj
                json.dump(convert_ndarray_to_list(scores), f)
                
        if 'npz' in args.export_mode:
            np.savez(
                scores_out_path,
                **scores,
            )
