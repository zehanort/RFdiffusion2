...
class PDBAtom:
    ...
    def __str__(
        ...
        # Format atom name correctly - this is not according to https://www.wwpdb.org/documentation/file-format-content/format33/sect9.html
        # but this makes it look like a normal pdb file one can download from the PDB
        # Example:
        # < 4 - like N, CA, C, O
        # ATOM      9  N   GLN A 147      57.662  24.770  19.354  1.00 25.21           N  
        # ATOM     10  CA  GLN A 147      57.613  25.033  20.783  1.00 26.40           C  
        # > 4 - like HE22
        # ATOM     20 HE22 GLN A 147      60.346  21.260  20.786  1.00  0.00           H  
        
        if len(self.atom_name.strip()) < 4:
            # Single or double-letter atom names get a leading space
            atom_name_formatted = f" {self.atom_name:<3}"
        else:
            # Four-letter atom names are left-aligned without leading space
            atom_name_formatted = f"{self.atom_name:<4}"

        # Construct the PDB ATOM line with precise column alignment
        atom_line = (
            f"{self.record_type:<6}"                   # Columns 1-6: Record name (e.g., "ATOM  ")
            f"{self.atom_index:>5} "                   # Columns 7-11: Atom serial number, right-aligned
            f"{atom_name_formatted}"                   # Columns 13-16: Atom name
            f"{self.alt_loc:1}"                        # Column 17: Alternate location indicator
            f"{self.res_name_3:>3} "                   # Columns 18-20: Residue name, right-aligned
            f"{self.chain_tag:1}"                      # Column 22: Chain identifier
            f"{self.residue_index:>4}"                 # Columns 23-26: Residue sequence number, right-aligned
            f"{self.insertion_code:1}   "              # Column 27: Insertion code + 3 spaces
            f"{self.pos[0]:>8.3f}"                     # Columns 31-38: X coordinate
            f"{self.pos[1]:>8.3f}"                     # Columns 39-46: Y coordinate
            f"{self.pos[2]:>8.3f}"                     # Columns 47-54: Z coordinate
            f"{self.occupancy:>6.2f}"                  # Columns 55-60: Occupancy
            f"{self.b_factor:>6.2f}          "         # Columns 61-66: B-factor + 10 spaces
            f"{self.element:>2}"                       # Columns 77-78: Element symbol, right-aligned
            f"{self.charge:>2}"                        # Columns 79-80: Charge
        )

        # Ensure the line is exactly 80 characters
        assert len(atom_line) == 80, f"PDB line length is {len(atom_line)} instead of 80: {atom_line}"
...
class PDBContext:
    ...
    def get_chain_is_smiles_ligand(self, asym_id: int) -> int:
        mask = self.token_asym_id == asym_id
        assert mask.sum() > 0
        e_type = self.token_entity_type[mask][0].item()
        assert isinstance(e_type, int)
        # if e_type != EntityType.LIGAND:
        #     return False

        res_names = self.token_residue_names[mask]
        atom_res_names_strs = [
            tensorcode_to_string(x)[:3].ljust(3) for x in res_names
        ]
        if e_type != EntityType.LIGAND:
            return False
        assert len(set(atom_res_names_strs)) == 1, f'{set(atom_res_names_strs)=}'
        return atom_res_names_strs[0] == 'LIG'
    ...
    def get_pdb_atoms(self):
        ...
        atom_entity_type = self.token_entity_type[self.atom_token_index]
        ...
        for atom_index in range(num_atoms):
            ...
            # Increment atom_index by 1 to start counting from 1
            pdb_atom_index = atom_index + 1
            is_ligand = atom_entity_type[atom_index].item() == EntityType.LIGAND.value

            atom = PDBAtom(
                record_type="ATOM" if not is_ligand else "HETATM",
                atom_index=pdb_atom_index,
...
def entity_to_pdb_atoms(entity: PDBContext) -> list[list[PDBAtom]]:
    ...
    for asym_id in chains:
        # deduplicate atom names within all ligand chains (only relevant for SMILES ligands)
        if entity.get_chain_entity_type(asym_id) == EntityType.LIGAND.value and entity.get_chain_is_smiles_ligand(asym_id):
